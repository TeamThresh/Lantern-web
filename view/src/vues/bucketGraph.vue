...
<script>
	module.exports = {
		...
		methods: {
			...
			draw: function(rawDataIndex) {
				rawDataIndex = rawDataIndex ? rawDataIndex : 44;
				...
				d3.json('/test/' + rawDataIndex, function(err, data) {
								...
								if( i > 0 ) {
									...
									if( sourceNode && destNode ) {
										var sourceIndex = nodes.indexOf(sourceNode),
											destIndex = nodes.indexOf(destIndex);
										var dup = false;
					...
					link = link.data(links)
				  	...
				  	node = node.data(nodes)
								...
								.attr("class", "node")
								.attr("r", 15)
								.attr('r', function(d) { return random(10, 30); })
